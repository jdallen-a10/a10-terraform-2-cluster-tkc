...
provider "kubernetes" {
  alias       = "cloud2"
  config_path = var.prov_config_path_2
}

#------------------------------------------------------------------#
resource "kubernetes_namespace" "blue" {
  provider = kubernetes.cloud2
  depends_on = [
...
#------------------------------------------------------------------#
resource "kubernetes_secret" "thunder-secret-2" {
  provider = kubernetes.cloud2
  metadata {
...
#------------------------------------------------------------------#
resource "kubernetes_service_account" "a10-sa-2" {
  provider = kubernetes.cloud2
  depends_on = [
    kubernetes_secret.thunder-secret-2
  ]
  metadata {
    name = "a10-sa"
  }
  secret {
    name = kubernetes_secret.thunder-secret-2.metadata[0].name
  }
}

#------------------------------------------------------------------#
resource "kubernetes_cluster_role_binding" "rbac-2" {
  depends_on = [
    kubernetes_service_account.a10-sa-2
  ]
  provider = kubernetes.cloud2
  metadata {
    name = "th-secret-rbac"
...
#------------------------------------------------------------------#
resource "kubernetes_config_map" "ws-config-2" {
  provider = kubernetes.cloud2
  depends_on = [
...
#------------------------------------------------------------------#
resource "kubernetes_config_map" "ws-index-2" {
  provider = kubernetes.cloud2
  depends_on = [
    kubernetes_namespace.blue
  ]
...
#------------------------------------------------------------------#
resource "kubernetes_deployment" "webservers-2" {
  provider = kubernetes.cloud2
  depends_on = [
    kubernetes_config_map.ws-config,
    kubernetes_config_map.ws-index-2
  ]
  metadata {
    name      = "webserver"
    namespace = kubernetes_namespace.blue.metadata[0].name
...
#------------------------------------------------------------------#
resource "kubernetes_service" "webserverSvc-2" {
  provider = kubernetes.cloud2
  depends_on = [
    kubernetes_deployment.webservers-2
  ]
  metadata {
    name      = "webserver-svc"
    namespace = kubernetes_namespace.blue.metadata[0].name
...
#------------------------------------------------------------------#
resource "kubernetes_ingress" "tkcIngress-2" {
  provider = kubernetes.cloud2
  depends_on = [
    kubernetes_deployment.webservers-2,
    kubernetes_service.webserverSvc-2
  ]
  metadata {
    ...
    annotations = {
      ...
      "acos.a10networks.com/health-monitors"             = "[{\"name\":\"ws-mon-2\", \"port\":\"80\",\"type\":\"http\"}]"
      "webserver-svc.acos.a10networks.com/service-group" = "{\"name\":\"ws-sg-2\",\"protocol\":\"tcp\",\"monitor\":\"ws-mon-2\",\"disableMonitor\":false}"
      "acos.a10networks.com/virtual-server"              = "{\"name\":\"ws-vip-2\",\"vip\":\"${var.thunder_vip_2}\"}"
...
#------------------------------------------------------------------#
resource "kubernetes_deployment" "tkc-2" {
  provider = kubernetes.cloud2
  depends_on = [
    kubernetes_ingress.tkcIngress-2,
    kubernetes_secret.thunder-secret-2,
    kubernetes_service_account.a10-sa-2,
    kubernetes_cluster_role_binding.rbac-2
  ]
  metadata {
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name = kubernetes_service_account.a10-sa-2.metadata[0].name
        container {
          name              = "thunder-kubernetes-connector"
          ...
          env {
            ...
            value = kubernetes_namespace.blue.metadata[0].name
          }
          env {
            name  = "CONTROLLER_URL"
          ...
          env {
            ...
            value = kubernetes_secret.thunder-secret-2.metadata[0].name
          }
          args = [
